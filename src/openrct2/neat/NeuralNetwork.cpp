...
void NeuralNetwork::Activate()
{
	...
	for (unsigned int i = m_num_inputs; i < m_neurons.size(); i++)
	{
		...
		double x = neuron.m_activesum;
		neuron.m_activesum = 0;

		// Apply the activation function
		double y = neuron.m_actFunc(x, neuron.m_a, neuron.m_b);

		neuron.m_activation = y;
	}
...
void NeuralNetwork::ActivateUseInternalBias()
{
	...
	for (unsigned int i = m_num_inputs; i < m_neurons.size(); i++)
	{
		...
		*/
		y = m_neurons[i].m_actFunc(x, m_neurons[i].m_a, m_neurons[i].m_b);
		m_neurons[i].m_activation = y;
	}
...
void NeuralNetwork::Adapt(NeatParameters& a_Parameters)
{
