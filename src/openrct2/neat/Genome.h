...
extern ActivationFunction GetRandomActivation(NeatParameters& a_Parameters, RNG& a_RNG);
...
class Genome
{
	...
	/////////////////////
private:
	std::unordered_map<unsigned int, size_t> m_idToIndexCache;
...
public:
	...
	bool IsCompatibleWith(Genome& a_G, NeatParameters& a_Parameters);

	// returns the absolute compatibility distance between this genome and a_G
	double CompatibilityDistance(Genome &a_G, NeatParameters& a_Parameters);
	...
	bool Mutate_AddNeuron(InnovationDatabase &a_Innovs, NeatParameters& a_Parameters, RNG& a_RNG);

	// Adds a new link to the genome
	// returns true if succesful
	bool Mutate_AddLink(InnovationDatabase &a_Innovs, NeatParameters& a_Parameters, RNG& a_RNG);
	...
	void Mutate_LinkWeights(NeatParameters& a_Parameters, RNG& a_RNG);
	...
	void Mutate_NeuronActivations_A(NeatParameters& a_Parameters, RNG& a_RNG);

	// Perturbs the B parameters of the neuron activation functions
	void Mutate_NeuronActivations_B(NeatParameters& a_Parameters, RNG& a_RNG);

	// Changes the activation function type for a random neuron
	void Mutate_NeuronActivation_Type(NeatParameters& a_Parameters, RNG& a_RNG);

	// Perturbs the neuron time constants
	void Mutate_NeuronTimeConstants(NeatParameters& a_Parameters, RNG& a_RNG);

	// Perturbs the neuron biases
	void Mutate_NeuronBiases(NeatParameters& a_Parameters, RNG& a_RNG);
	...
	void BuildESHyperNEATPhenotype(NeuralNetwork& a_net, Substrate& subst, NeatParameters& params);
	void DivideInitialize(const std::vector<double>& node,
						  std::shared_ptr<QuadPoint>& root,
						  NeuralNetwork& cppn, NeatParameters& params,
	...
	void PruneExpress(const std::vector<double>& node,
					  ...
					  NeatParameters& params, std::vector<Genome::TempConnection>& connections,
					  const bool& outgoing);
