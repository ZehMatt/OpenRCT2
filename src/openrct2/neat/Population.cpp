...
// The constructor
Population::Population(const Genome& a_Seed, const NeatParameters& a_Parameters,
		               bool a_RandomizeWeights, double a_RandomizationRange, int a_RNG_seed)
{
	...
	//printf("RNG State: %u\n", m_RNG.RandInt(0, 10000));

    // Speciate
    Speciate();

	//printf("(Speciate) RNG State: %u\n", m_RNG.RandInt(0, 10000));
...
{   
   ...
   // for(unsigned int i=0; i<m_Species.size(); i++) m_Species[i].KillWorst(m_Parameters);
	//printf("(Before Reproduction) RNG State: %u\n", m_RNG.RandInt(0, 10000));
    ...
    for(unsigned int i=0; i<m_Species.size(); i++)
    {
		...
		//printf("(Repo: %u) RNG State: %u\n", repo++, m_RNG.RandInt(0, 10000));
    }
    m_Species = m_TempSpecies;

	//printf("(After Reproduction) RNG State: %u\n", m_RNG.RandInt(0, 10000));
   ...
   // for(unsigned int i=0; i<m_Species.size(); i++) m_Species[i].KillOldParents();
	...
	//printf("(After EPOCH) RNG State: %u\n", m_RNG.RandInt(0, 10000));
}
