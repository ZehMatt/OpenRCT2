...
Genome Species::GetIndividual(NeatParameters& a_Parameters, RNG& a_RNG) const
{
    ...
    {   //start with the last one just for comparison sake
        ...
        int t_num_parents = floor((a_Parameters.SurvivalRate * (static_cast<double>(t_Evaluated.size())))+1.0);
...
void Species::AdjustFitness(NeatParameters& a_Parameters)
{
    ASSERT(m_Individuals.size() > 0);
...
void Species::Reproduce(Population &a_Pop, NeatParameters& a_Parameters, RNG& a_RNG)
{
...
Genome Species::ReproduceOne(Population& a_Pop, NeatParameters& a_Parameters, RNG& a_RNG)
{
...
// Mutates a genome
void Species::MutateGenome( bool t_baby_is_clone, Population &a_Pop, Genome &t_baby, NeatParameters& a_Parameters, RNG& a_RNG )
{
#if 1
    // NEW version:
