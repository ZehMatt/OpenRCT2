...
// forward
ActivationFunction GetRandomActivation(NeatParameters& a_Parameters, RNG& a_RNG);
...
Genome::Genome(const Genome& a_G)
{
	...
	m_idToIndexCache = a_G.m_idToIndexCache;
...
Genome::Genome(unsigned int a_ID,
			   ...
			   const NeatParameters& a_Parameters)
{
...
int Genome::GetNeuronIndex(unsigned int a_id) const
{
	...
	if (m_idToIndexCache.empty() == false)
	{
		auto itr = m_idToIndexCache.find(a_id);
		if (itr != m_idToIndexCache.end())
...
void Genome::RebuildIDIndexCache()
{
	/*
	m_idToIndexCache.clear();
	m_idToIndexCache.reserve(NumNeurons());

	for (size_t i = 0; i < NumNeurons(); i++)
	{
		unsigned int id = m_NeuronGenes[i].ID();
		m_idToIndexCache.insert(std::make_pair(id, i));
	}
	*/
}
...
void Genome::BuildPhenotype(NeuralNetwork& a_Net) const
{
	...
	for (unsigned int i = 0; i<NumNeurons(); i++)
	{
		...
		t_n.m_actFunc = ActivationFunctions[t_n.m_activation_function_type];
...
void Genome::BuildHyperNEATPhenotype(NeuralNetwork& net, Substrate& subst)
{
	...
	size_t max_dims = subst.GetMaxDims();
	...
	for (unsigned int i = 0; i<subst.m_input_coords.size(); i++)
	{
		...
		t_n.m_activation_function_type = NEAT::ActivationFunction::LINEAR;
		t_n.m_actFunc = ActivationFunctions[t_n.m_activation_function_type];
	...
	std::vector< std::vector<size_t> > t_to_query;
	...
	{
		size_t j = t_to_query[conn][0];
		size_t i = t_to_query[conn][1];
		...
		size_t from_dims = net.m_neurons[j].m_substrate_coords.size();
		size_t to_dims = net.m_neurons[i].m_substrate_coords.size();

		// input the node positions to the CPPN
		// from
		for (size_t n = 0; n<from_dims; n++)
		{
			t_inputs[n] = net.m_neurons[j].m_substrate_coords[n];
		}
		// to
		for (size_t n = 0; n<to_dims; n++)
		{
		...
		if (((t_link > 0) && (!subst.m_query_weights_only)) || (subst.m_query_weights_only))
		{
			...
			t_c.m_source_neuron_idx = (unsigned short)j;
			t_c.m_target_neuron_idx = (unsigned short)i;
...
double Genome::CompatibilityDistance(Genome &a_G, NeatParameters& a_Parameters)
{
	...
	{
		if (t_g1 == m_LinkGenes.end()) // end of first genome?
		{
			// add to the total distance
			t_num_excess++;
			t_g2++;
		}
		else if (t_g2 == a_G.m_LinkGenes.end()) // end of second genome?
		{
			// add to the total distance
			t_num_excess++;
			t_g1++;
	...
	for (unsigned int i = 0; i < NumNeurons(); i++)
	{
		...
		NeuronGene& gene = m_NeuronGenes[i];
		if ((gene.Type() != INPUT) && (gene.Type() != BIAS))
		{
			// a match
			if (a_G.HasNeuronID(gene.ID()))
			{
				t_num_matching_neurons++;

				double t_A_difference = gene.m_A - a_G.GetNeuronByID(gene.ID()).m_A;
				if (t_A_difference < 0.0f) t_A_difference = -t_A_difference;
				t_total_A_difference += t_A_difference;

				double t_B_difference = gene.m_B - a_G.GetNeuronByID(gene.ID()).m_B;
				if (t_B_difference < 0.0f) t_B_difference = -t_B_difference;
				t_total_B_difference += t_B_difference;

				double t_time_constant_difference = gene.m_TimeConstant - a_G.GetNeuronByID(gene.ID()).m_TimeConstant;
				...
				double t_bias_difference = gene.m_Bias - a_G.GetNeuronByID(gene.ID()).m_Bias;
				...
				if (gene.m_ActFunction != a_G.GetNeuronByID(gene.ID()).m_ActFunction)
				{
...
bool Genome::IsCompatibleWith(Genome& a_G, NeatParameters& a_Parameters)
{
...
bool Genome::Mutate_AddNeuron(InnovationDatabase &a_Innovs, NeatParameters& a_Parameters, RNG& a_RNG)
{
...
// returns true if succesful
bool Genome::Mutate_AddLink(InnovationDatabase &a_Innovs, NeatParameters& a_Parameters, RNG& a_RNG)
{
...
void Genome::Mutate_LinkWeights(NeatParameters& a_Parameters, RNG& a_RNG)
{
#if 1
...
void Genome::Mutate_NeuronActivations_A(NeatParameters& a_Parameters, RNG& a_RNG)
{
	// for all neurons..
	for (unsigned int i = 0; i<NumNeurons(); i++)
	{
...
void Genome::Mutate_NeuronActivations_B(NeatParameters& a_Parameters, RNG& a_RNG)
{
	// for all neurons..
	for (unsigned int i = 0; i<NumNeurons(); i++)
	{
...
void Genome::Mutate_NeuronActivation_Type(NeatParameters& a_Parameters, RNG& a_RNG)
{
	// the first non-input neuron
	int t_first_idx = NumInputs();
	int t_choice = a_RNG.RandInt((int)t_first_idx, (int)m_NeuronGenes.size() - 1);

	NeuronGene& choice = m_NeuronGenes[t_choice];

	choice.m_ActFunction = GetRandomActivation(a_Parameters, a_RNG);
	choice.m_ActFunc = ActivationFunctions[choice.m_ActFunction];
}

// Perturbs the neuron time constants
void Genome::Mutate_NeuronTimeConstants(NeatParameters& a_Parameters, RNG& a_RNG)
{
	// for all neurons..
	for (unsigned int i = 0; i<NumNeurons(); i++)
	{
...
void Genome::Mutate_NeuronBiases(NeatParameters& a_Parameters, RNG& a_RNG)
{
	// for all neurons..
	for (unsigned int i = 0; i<NumNeurons(); i++)
	{
...
Genome::Genome(std::ifstream& a_DataFile)
{
	...
	do
	{
		...
		if (t_Str == "Link")
		{
			...
			m_LinkGenes.push_back(LinkGene(t_from, t_to, t_innov, t_weight, !!t_isrecur));
		}
...
void Genome::BuildESHyperNEATPhenotype(NeuralNetwork& net, Substrate& subst, NeatParameters& params)
{
	ASSERT(subst.m_input_coords.size() > 0);
	ASSERT(subst.m_output_coords.size() > 0);

	unsigned int input_count = (unsigned int)subst.m_input_coords.size();
	unsigned int output_count = (unsigned int)subst.m_output_coords.size();
	...
	struct RangeHash
	{
		std::size_t operator()(const std::vector<double>& c) const
		{
			std::size_t n = 0;
			std::hash<double> h;
	...
	std::unordered_map< std::vector<double>, int, RangeHash > hidden_nodes;
	hidden_nodes.reserve(maxNodes);

	std::unordered_map< std::vector<double>, int, RangeHash > temp;
	temp.reserve(maxNodes);

	std::unordered_map< std::vector<double>, int, RangeHash > unexplored_nodes;
	...
	{
		std::unordered_map< std::vector<double>, int, RangeHash >::iterator itr_hid;
		...
		std::unordered_map< std::vector<double>, int, RangeHash >::iterator itr1;
	...
	std::unordered_map< std::vector<double>, int, RangeHash>::iterator itr;
	...
	Clean_Net(net.m_connections, input_count, output_count, (unsigned int)hidden_nodes.size());
}
...
double Genome::Variance(std::shared_ptr<QuadPoint> &point)
{
	...
	for (int i = 0; i < 4; i++)
	{
		sum += (point->children[i]->weight - mean) * (point->children[i]->weight - mean);
	}
	sum /= 4;
	...
	/*
	std::accumulators::accumulator_set<double, boost::accumulators::stats< boost::accumulators::tag::variance> > acc;
	for (unsigned int i = 0; i < 4; i++)
	{
