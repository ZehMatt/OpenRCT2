        ...
        std::vector<std::vector<double> >& a_outputs)
{
    ...
    m_with_distance = false;
    m_query_weights_only = false;
    m_hidden_nodes_activation = NEAT::ActivationFunction::UNSIGNED_SIGMOID;
    m_output_nodes_activation = NEAT::ActivationFunction::UNSIGNED_SIGMOID;
    m_allow_input_hidden_links = true;
...
size_t Substrate::GetMinCPPNInputs()
{
    // determine the dimensionality across the entire substrate
    size_t cppn_inputs = GetMaxDims() * 2; // twice, because we query 2 points at a time
...
size_t Substrate::GetMinCPPNOutputs()
{
	size_t outs = 0;
