...
{
    _hostInfo.name = String::ToStd(gConfigNetwork.server_name);
    _hostInfo.description = String::ToStd(gConfigNetwork.server_description);
    _hostInfo.greeting = String::ToStd(gConfigNetwork.server_greeting);
    _hostInfo.providerName = String::ToStd(gConfigNetwork.provider_name);
    _hostInfo.providerEmail = String::ToStd(gConfigNetwork.provider_email);
    _hostInfo.providerWebsite = String::ToStd(gConfigNetwork.provider_website);
}
...
void NetworkServer::AcceptClients()
{
    while (true)
    {
        std::unique_ptr<ITcpSocket> tcpSocket = _listenSocket->Accept2();
        if (!tcpSocket)
            break;
...
void NetworkServer::AcceptClient(std::unique_ptr<ITcpSocket>&& sock)
{
    ...
    std::unique_ptr<NetworkConnection> connection = std::make_unique<NetworkConnection>();
    connection->Sock = std::move(sock);
